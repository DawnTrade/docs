...
Tapping **⚙️ Automations** unfolds one more row:

| Control | What it does |
| --- | --- |
| 📋 **Limit Order** | A buy or a sell that waits for a market cap you set. |
...
Dawn remembers whether you left the row open, so if you use orders it stays unfolded on every panel.
...
When something is already waiting on the coin, the panel says so in a line of its own above the buttons — and it shows the orders themselves, not just a count:

```text
📋 Buy $5 @ $170K, $10 @ $90K · 🎯 TP $1.2M · SL $340K
```

Limit buys come first, each as its size and the market cap it is waiting for, oldest first. Take-profit and stop-loss levels follow. Past two of either, the rest are summed as `+3` rather than listed — the line has one row to live in.

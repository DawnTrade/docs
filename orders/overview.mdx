...
description: "Every kind of automation, and which one matches what you are trying to do."
icon: "crosshairs"
---
...
| Split a big buy or sell into small trades over time | [TWAP](/orders/twap) |
...
| Just be told when a coin reaches a level | [Price alert](/alerts/price) |
| Do what another wallet does, as it does it | [Copytrade](/copytrade/overview) |
...
You can type shorthand — `250k`, `1.5m`, `\$2m` all work. A limit order also takes a move from the market cap right now — `2x`, `+50%`, `-30%` — and works out the level for you.
...
- **They survive a profile switch.** Check `/orders` in each profile before assuming nothing is armed.
